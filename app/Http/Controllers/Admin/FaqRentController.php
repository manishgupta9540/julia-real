...
class FaqRentController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $data = FaqRent::select('id','title','created_at','status')->whereIn('status',[0,1])->get();
            ...
            ->addColumn('action', function($row){
                ...
                $edit_url = url('/admin/faq-rent/edit',['id'=>base64_encode($row->id)]);
        ...
        return view('admin.faqrent.index');
    }
    ...
    public function create(Request $request)
    {
        ...
        {
            return view('admin.faqrent.create');
        }
        ...
        try{
            ...
            DB::table('faq_rents')->insert($data);
    ...
    public function faqrentEdit(Request $request)
    {
        $faq_id = base64_decode($request->id);

        $faq_data  = DB::table('faq_rents')->where('id',$faq_id)->first();
        return view('admin.faqrent.edit',compact('faq_data'));
    }

    public function fqaUpdate(Request $request)
    {
        $faq_id = base64_decode($request->id);
        ...
        try{
            ...
            DB::table('faq_rents')->where('id',$faq_id)->update($data);
    ...
    public function faqrentDelete(Request $request)
    {
        $faq_id = base64_decode($request->id);
        DB::beginTransaction();
        try{
            $blog = FaqRent::find($faq_id);
            $blog->status = '2';
            $blog->save();
