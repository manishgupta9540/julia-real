...
class WhyChooseController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = WhyChoose::select('id','description','image','created_at','status')->whereIn('status',[0,1])->get();
            ...
            ->addColumn('action', function($row){
                ...
                $edit_url = url('/admin/why-choose/edit',['id'=>base64_encode($row->id)]);
    ...
    public function create(Request $request)
    {
        ...
        try{
            ...
            DB::table('why_chooses')->insert($data);
    ...
    public function whyChooseEdit(Request $request)
    {
        $choose_id = base64_decode($request->id);

        $choose  = DB::table('why_chooses')->where('id',$choose_id)->first();
        $choosesibgle  = DB::table('why_chooses')->where('id',$choose_id)->first();
        return view('admin.why_choose.edit',compact('choose','choosesibgle'));
    }

    public function deletedWhyChosse(Request $request)
    {
        ...
        try{
            if(Auth::check()){
                ...
                $why_choose = DB::table('why_chooses')->where(['id'=>$choose_id])->first();
                ...
                DB::table('why_chooses')->where(['id'=>$choose_id])->update([
    ...
    public function whyChooseUpdate(Request $request)
    {
        ...
        $old_image = DB::table('why_chooses')->where('id',$choose_id)->first();

        DB::beginTransaction();
        try{

            $data1 =   DB::table('why_chooses')->where('id',$choose_id)->first();
            $status = $data1->status;
            ...
            DB::table('why_chooses')->where('id',$choose_id)->update($data);
    ...
    public function whyChooseStatus(Request $request)
    {
        $choose_id=base64_decode($request->id);
