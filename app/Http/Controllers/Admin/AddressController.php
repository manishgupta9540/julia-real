...
class AddressController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = Address::select('id','address','contact','email','created_at','status')->whereIn('status',[0,1])->get();
            ...
            ->addColumn('action', function($row){
                ...
                $edit_url = url('/admin/address/edit',['id'=>base64_encode($row->id)]);
    ...
    public function addressUpdate(Request $request)
    {
        ...
        try{
            ...
            $data1 =   DB::table('addresses')->where('id',$blog_id)->first();
            $status = $data1->status;
            ...
            DB::table('addresses')->where('id',$blog_id)->update($data);
