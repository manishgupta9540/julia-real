...
class FaqController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = Faq::select('id','title','created_at','status')->whereIn('status',[0,1])->get();
    ...
    public function faqEdit(Request $request)
    {
        ...
        $faq_data  = DB::table('faqs')->where('id',$faq_id)->first();
        return view('admin.faq.edit',compact('faq_data'));
    }
    ...
    public function fqaUpdate(Request $request)
    {
        ...
        try{
            ...
            $data1 =   DB::table('faqs')->where('id',$faq_id)->first();
            $status = $data1->status;
    ...
    public function fqaDelete(Request $request)
    {
        $faq_id = base64_decode($request->id);
        DB::beginTransaction();
        try{
            $blog = Faq::find($faq_id);
            $blog->status = '2';
            $blog->save();
