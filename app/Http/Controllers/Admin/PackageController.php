...
class PackageController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = Package::select('id','package_name','price','description','created_at','status')->whereIn('status',[0,1])->get();
            ...
            ->addColumn('action', function($row){
                ...
                $edit_url = url('/admin/package/edit',['id'=>base64_encode($row->id)]);
    ...
    {
        $package_id = base64_decode($request->id);

        $packages  = DB::table('packages')->where('id',$package_id)->first();
    ...
    public function packageUpdate(Request $request)
    {
        ...
        try{
            ...
            $data1 =   DB::table('packages')->where('id',$package_id)->first();
            $status = $data1->status;
